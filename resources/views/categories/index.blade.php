...
<div class="container py-4">
    <div class="d-flex justify-content-between align-items-center flex-wrap mb-4">
        <h2 class="mb-2">🏷️ Gestion des Catégories de Produits</h2>
    ...
    {{-- Section de filtrage --}}
    <form action="{{ route('categories.index') }}" method="GET" class="mb-4 p-4 bg-white rounded-4 shadow-sm">
        <div class="row g-3 align-items-end">
            <div class="col-md-10">
                <label for="search" class="form-label">Recherche par nom ou description</label>
                <input type="text" class="form-control" id="search" name="search" placeholder="Nom ou description de la catégorie..." value="{{ request('search') }}">
            </div>
            <div class="col-md-2 d-grid gap-2">
                <button type="submit" class="btn btn-info"><i class="fas fa-filter me-1"></i> Filtrer</button>
                <a href="{{ route('categories.index') }}" class="btn btn-outline-secondary"><i class="fas fa-sync-alt me-1"></i> Réinitialiser</a>
            </div>
        </div>
    </form>
                ...
                <table class="table table-hover align-middle mb-0">
                    ...
                    <tbody>
                        @forelse ($categories as $category)
                            <tr>
                                <td>{{ $category->name }}</td>
                                <td class="{{ $category->description ? '' : 'text-not-set' }}">{{ $category->description ?? 'Non renseigné' }}</td>
                            ...
                            @include('categories.partials.edit_modal', ['category' => $category])
                            @include('categories.partials.delete_modal', ['category' => $category])
...
{{ $categories->links('vendor.pagination.bootstrap-5') }}    </div>
    </div>
</div>

{{-- Modale d'ajout de catégorie --}}
@include('categories.partials.create_modal')
