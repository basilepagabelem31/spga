...
<div class="container py-4">
    <div class="d-flex justify-content-between align-items-center flex-wrap mb-4">
        <h2 class="mb-2">🔑 Gestion des Permissions</h2>
        <button type="button" class="btn btn-primary shadow-sm" data-bs-toggle="modal" data-bs-target="#createPermissionModal">
            <i class="fas fa-plus-circle me-1"></i> Ajouter une permission
        </button>
    </div>
    ...
    {{-- Section de filtrage --}}
    <form action="{{ route('permissions.index') }}" method="GET" class="mb-4 p-4 bg-white rounded-4 shadow-sm">
        <div class="row g-3 align-items-end">
            <div class="col-md-10">
                <label for="search" class="form-label">Recherche par nom ou description</label>
                <input type="text" class="form-control" id="search" name="search" placeholder="Nom ou description de la permission..." value="{{ request('search') }}">
            </div>
            <div class="col-md-2 d-grid gap-2">
                <button type="submit" class="btn btn-info"><i class="fas fa-filter me-1"></i> Filtrer</button>
                <a href="{{ route('permissions.index') }}" class="btn btn-outline-secondary"><i class="fas fa-sync-alt me-1"></i> Réinitialiser</a>
            </div>
        </div>
    </form>
                    ...
                    <tbody>
                        @forelse ($permissions as $permission)
                            <tr>
                                ...
                                <td class="{{ $permission->description ? '' : 'text-not-set' }}">{{ $permission->description ?? 'Non renseigné' }}</td>
                                <td class="text-end pe-4">
                                    <div class="btn-group" role="group">
                                        <!-- Modifier -->
                                        <button type="button" class="btn btn-sm btn-outline-warning" data-bs-toggle="modal" data-bs-target="#editPermissionModal{{ $permission->id }}" title="Modifier la permission">
                                            <i class="fas fa-edit"></i>
                                        </button>

                                        <!-- Supprimer -->
                                        <button type="button" class="btn btn-sm btn-outline-danger" data-bs-toggle="modal" data-bs-target="#deletePermissionModal{{ $permission->id }}" title="Supprimer la permission">
                            ...
                            {{-- Modales pour chaque permission --}}
                            @include('permissions.partials.edit_modal', ['permission' => $permission])
                            @include('permissions.partials.delete_modal', ['permission' => $permission])

                        @empty
                            <tr>
                                <td colspan="3" class="text-center text-muted py-4">Aucune permission trouvée.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
...
{{-- Modale d'ajout de permission --}}
@include('permissions.partials.create_modal')
