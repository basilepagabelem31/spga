...
<style>
    ...
    .table thead th:nth-child(4), /* Mode Prod. */
    .table tbody td:nth-child(4) {
        min-width: 120px;
    }

    .table thead th:nth-child(5), /* Prix Unitaire */
    .table tbody td:nth-child(5) {
        min-width: 120px;
    }

    .table thead th:nth-child(6), /* Unité Vente */
    .table tbody td:nth-child(6) {
        min-width: 100px;
    }
    
    .table thead th:nth-child(7), /* Stock Actuel */
    .table tbody td:nth-child(7) {
        min-width: 120px;
    }

    .table thead th:nth-child(8), /* Statut */
...
<div class="container py-4">
    ...
    {{-- Section de filtrage --}}
    <form action="{{ route('partenaire.products') }}" method="GET" class="mb-4 p-4 bg-white rounded-4 shadow-sm">
        <div class="row g-3 align-items-end">
            ...
            <div class="col-md-4 d-grid">
                <a href="{{ route('partenaire.products') }}" class="btn btn-outline-secondary"><i class="fas fa-sync-alt me-1"></i> Réinitialiser</a>
            </div>
        </div>
    </form>
                    ...
                    <tbody>
                        @forelse ($products as $product)
                            <tr>
                                ...
                                <td>
                                    <form action="{{ route('partenaire.products.updateStock', $product) }}" method="POST" class="stock-form">
    @csrf
    @method('PUT')
    <div class="input-group input-group-sm">
        {{-- Le nom du champ a été changé ici --}}
        <input type="number" name="quantity" value="{{ old('current_stock_quantity', number_format($product->current_stock_quantity, 2, '.', '')) }}" class="form-control" min="0" step="0.01" style="width: 80px;">
        <button type="submit" class="btn btn-sm btn-outline-primary" title="Mettre à jour le stock">
            <i class="fas fa-save"></i>
                            ...
                            @include('partners.products.partials.edit_modal', ['product' => $product, 'categories' => $categories])
                            @include('partners.products.partials.delete_modal', ['product' => $product])
...
@include('partners.products.partials.create_modal', ['categories' => $categories])
