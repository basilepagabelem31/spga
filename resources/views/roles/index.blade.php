...
<div class="container py-4">
    <div class="d-flex justify-content-between align-items-center flex-wrap mb-4">
        <h2 class="mb-2">👥 Gestion des Rôles</h2>
        <button type="button" class="btn btn-primary shadow-sm" data-bs-toggle="modal" data-bs-target="#createRoleModal">
    ...
    {{-- Section de filtrage --}}
    <form action="{{ route('roles.index') }}" method="GET" class="mb-4 p-4 bg-white rounded-4 shadow-sm">
        <div class="row g-3 align-items-end">
            <div class="col-md-10">
                <label for="search" class="form-label">Recherche par nom ou description</label>
                <input type="text" class="form-control" id="search" name="search" placeholder="Nom ou description du rôle..." value="{{ request('search') }}">
            </div>
            <div class="col-md-2 d-grid gap-2">
                <button type="submit" class="btn btn-info"><i class="fas fa-filter me-1"></i> Filtrer</button>
                <a href="{{ route('roles.index') }}" class="btn btn-outline-secondary"><i class="fas fa-sync-alt me-1"></i> Réinitialiser</a>
            </div>
        </div>
    </form>
                ...
                <table class="table table-hover align-middle mb-0">
                    ...
                    <tbody>
                        @forelse ($roles as $role)
                            <tr>
                                <td>{{ $role->name }}</td>
                                <td class="{{ $role->description ? '' : 'text-not-set' }}">{{ $role->description ?? 'Non renseigné' }}</td>
                                <td class="text-end pe-4">
                                    <div class="btn-group" role="group">
                                        <!-- Modifier -->
                                        <button type="button" class="btn btn-sm btn-outline-warning" data-bs-toggle="modal" data-bs-target="#editRoleModal{{ $role->id }}" title="Modifier le rôle">
                                            <i class="fas fa-edit"></i>
                                        </button>

                                        <!-- Supprimer -->
                                        <button type="button" class="btn btn-sm btn-outline-danger" data-bs-toggle="modal" data-bs-target="#deleteRoleModal{{ $role->id }}" title="Supprimer le rôle">
                            ...
                            {{-- Modales pour chaque rôle --}}
                            @include('roles.partials.edit_modal', ['role' => $role])
                            @include('roles.partials.delete_modal', ['role' => $role])

                        @empty
                            <tr>
                                <td colspan="3" class="text-center text-muted py-4">Aucun rôle trouvé.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
...
@include('roles.partials.create_modal')
